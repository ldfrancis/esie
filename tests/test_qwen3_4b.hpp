...
int test_qwen3_4b() {
    ...
    std::ifstream file("../weights/Qwen_Qwen3-4B_weights_fp32.bin", std::ios::in|std::ios::binary);
    if(!file.is_open()){
        std::cerr << "Error opening file!" << std::endl;
        return -1;
    }
    std::cout<<"Loading weights from file ../weights/Qwen_Qwen3-4B_weights_fp32.bin" << std::endl;
    ...
    // load logits
    std::cout << "Loading target logits from file ../weights/Qwen_Qwen3-4B_logits.bin" << std::endl;
    std::ifstream file2("../weights/Qwen_Qwen3-4B_logits.bin", std::ios::in|std::ios::binary);
    if(!file2.is_open()){
        std::cerr << "Error opening file!" << std::endl;
        return -1;
    }
