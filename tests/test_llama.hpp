...
int test_llama() {
    ...
    std::ifstream file("../weights/meta-llama_Llama-2-7b-hf_weights_fp32.bin", std::ios::in|std::ios::binary);
    if(!file.is_open()){
    ...
    // load logits
    std::ifstream file2("../weights/meta-llama_Llama-2-7b-hf_logits.bin", std::ios::in|std::ios::binary);
    if(!file2.is_open()){
        std::cerr << "Error opening file!" << std::endl;
        return -1;
    }
